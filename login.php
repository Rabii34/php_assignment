...
if(isset($_POST['login'])){
   ...
   $log_email_check="SELECT * from `admin_signup` where email='$log_email'";
   $log_query=mysqli_query($connection,$log_email_check);
   if(mysqli_num_rows($log_query)>0){
      $row=mysqli_fetch_assoc($log_query);
     $log_dbpass=$row['password'];
     $verify_pass=password_verify($log_pass,$log_dbpass);
     if($verify_pass){
      session_start();
